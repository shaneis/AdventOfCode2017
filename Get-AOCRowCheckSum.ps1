function Get-AOCRowCheckSum
{
    [CmdletBinding()]
    param (
        # Input value of the table
        [Parameter(Mandatory)]
        $InputRow
    )
    ...
    process
    {
        $SplitRows = $InputRow -split [System.Environment]::NewLine

        foreach ($line in $SplitRows)
        {
            $MeasuredObject = $line -split {$_ -eq ' ' -or $_ -eq "`t" } | Measure-Object -Maximum -Minimum

            $i++

            [PSCustomObject]@{
                Max        = $MeasuredObject.Maximum
                Min        = $MeasuredObject.Minimum
                Difference = $MeasuredObject.Maximum - $MeasuredObject.Minimum
                Index = $i
            }
        }
    }
...
#Get-AOCRowCheckSum -InputRow '7 5 3'


Get-AOCRowCheckSum -InputRow @"
